...
//         {!isMe && <Skeleton width={60} height={12} style={{ marginBottom: 4 }} />}
//         <Skeleton width={Math.random() * 100 + 100} height={14} style={{ marginBottom: 4 }} />
...
const GroupChatContent = ({ groupId }: { groupId: string }) => {
  ...
  // Swipe-to-Reply Component
  const SwipeableMessage = ({ msg, children }: { msg: any; children: React.ReactNode }) => {
    ...
    const panGesture = Gesture.Pan()
      .activeOffsetX(10)
      .onUpdate((event) => {
        if (event.translationX > 0 && event.translationX < 80 && !msg.isMe) {
      ...
      .onEnd((event) => {
        if (event.translationX > 50 && !msg.isMe && msg.rawMessage) {
          startReply(msg.rawMessage);
        }
    ...
    const replyIconStyle = useAnimatedStyle(() => ({
      ...
      transform: [
        { translateX: interpolate(translateX.value, [0, 60], [-30, 0], Extrapolation.CLAMP) }
      ],
    }));

    if (msg.isMe) {
      // Don't add swipe gesture for own messages
      return <>{children}</>;
    }
    ...
    return (
      <GestureDetector gesture={panGesture}>
        <Animated.View style={[styles.swipeContainer]}>
          <Animated.View style={[replyIconStyle, styles.replyIcon]}>
