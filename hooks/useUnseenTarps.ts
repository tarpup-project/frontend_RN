...
export const useUnseenTarps = () => {
    ...
    const checkUnseen = async () => {
        ...
        try {
            // 1. Get previewed posts
            const previewedJson = await AsyncStorage.getItem('previewedPosts');
            ...
            // Simplified: just check if any ID is NOT in previewed
            // Note: The structure might be grid cells or direct posts. 
            // We need to extract all individual post IDs.
            ...
            // 4. Count unseen
            const unseen = allIds.filter(id => !previewed.has(id)).length;
            console.log(`🌍 Global unseen check: ${unseen} unseen posts out of ${allIds.length} total`);
